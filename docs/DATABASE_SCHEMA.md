...
v3.1.2 added one defaulted cycle-state column: `buy_remainder_cancel_requested INTEGER NOT NULL DEFAULT 0`. v3.2.0 added no table or column; its one-database currency lock uses the existing `app_settings` table and is inferred from historical cycles when upgrading. v3.2.1 also adds no table, column, or index. The new `PreflightBlocked` value is stored in the existing text status fields. v3.2.2 is GUI-only and likewise adds no table, column, index, or persisted contract-description field. v3.3.0 adds system-theme presentation and packaging changes only, with no SQLite object or persisted theme setting. v3.4.0 adds reliability and recovery corrections only, with no SQLite object or persisted setting. v3.5.0 changes GUI layout and runtime theme handling only, with no SQLite object or persisted setting. v3.6.0 changes SELL reconciliation and Trade History behavior only, with no SQLite object or persisted setting. v3.7.0 adds field-level market-data and Stage-3 SELL safeguards only, with no SQLite object, migration, index, or persisted setting. v3.8.0 changes the runtime BUY partial-fill cancellation policy only; it reuses the existing `buy_filled_at` timestamp and `buy_remainder_cancel_requested` flag and adds no SQLite object, migration, index, or persisted setting. v3.9.0 changes only in-memory diagnostic coalescing, structured audit-event payloads, and GUI status presentation; it adds no SQLite object, migration, index, or persisted setting. Existing v3.5.0, v3.2.2, v3.2.0, v3.1.2, v3.1.1, v3.1.0, and v3.0.19 databases open through the normal additive path; a database with valid single-currency history keeps all existing rows.
