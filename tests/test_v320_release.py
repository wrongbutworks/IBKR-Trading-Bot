"""v3.2.0 feature and current v3.4.0 release regressions."""
...
def test_v320_release_note_is_archived_and_v322_is_current() -> None:
    ...
    assert "V3_4_0_RELIABILITY_AND_RECOVERY_FIXES.md" in README
    assert "V3_4_0_RELIABILITY_AND_RECOVERY_FIXES.md" in DOCS_INDEX
...
def test_v320_current_verification_documents_match_the_release() -> None:
    assert "verification scope for v3.4.0" in AUTOMATED_COVERAGE
    assert "v3.2.0 exact-contract, currency, and reconnect layer" in AUTOMATED_COVERAGE
    assert "current non-GUI, non-Windows, non-network test layer in v3.4.0" in OFFLINE_TESTS
    assert "v3.2.0 USD/EUR SMART and reconnect regressions" in TESTING_GUIDE
    assert "This v3.4.0 test-only matrix" in CSV_MATRIX
